...
const prepareWAMessageMedia = async (message, options) => {
    ...
    if (mediaType === 'audio' && (uploadData.ptt || message.forceOpus)) {
...
const generateWAMessageContent = async (message, options) => {
    ...
    if ('buttons' in message && Array.isArray(message.buttons)) {
        const hasNativeFlow = message.buttons.some((b) => b.type === 4 || b.nativeFlowInfo);
        ...
        if ('image' in message || 'video' in message || 'document' in message) {
            const mediaMsg = await (0, exports.prepareWAMessageMedia)(message, options);
            interactiveMessage.header.hasMediaAttachment = true;
            if ('image' in message)
                interactiveMessage.header.imageMessage = mediaMsg.imageMessage;
            if ('video' in message)
                interactiveMessage.header.videoMessage = mediaMsg.videoMessage;
            if ('document' in message)
                interactiveMessage.header.documentMessage = mediaMsg.documentMessage;
        }
        m = { viewOnceMessage: { message: { interactiveMessage } } };
        if ('contextInfo' in message) {
            m.viewOnceMessage.message.interactiveMessage.contextInfo = message.contextInfo;
        }
        else if ('mentions' in message) {
            m.viewOnceMessage.message.interactiveMessage.contextInfo = { mentionedJid: message.mentions };
        }
        return m;
    }
    if ('text' in message) {
