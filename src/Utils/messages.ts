...
export const generateWAMessageContent = async (
    ...
    // [XBATS PATCH] Intercept legacy buttons
    if ('buttons' in message && Array.isArray((message as any).buttons)) {
        const hasNativeFlow = (message as any).buttons.some((b: any) => b.type === 4 || b.nativeFlowInfo)
        ...
        // Handle media
        if ('image' in message || 'video' in message || 'document' in message) {
            const mediaMsg = await prepareWAMessageMedia(message as any, options)
            interactiveMessage.header.hasMediaAttachment = true
            if ('image' in message) interactiveMessage.header.imageMessage = mediaMsg.imageMessage
            if ('video' in message) interactiveMessage.header.videoMessage = mediaMsg.videoMessage
            if ('document' in message) interactiveMessage.header.documentMessage = mediaMsg.documentMessage
        }

        m = { viewOnceMessage: { message: { interactiveMessage } } } as any
        
        if ('contextInfo' in message) {
            (m as any).viewOnceMessage.message.interactiveMessage.contextInfo = (message as any).contextInfo
        } else if ('mentions' in message) {
            (m as any).viewOnceMessage.message.interactiveMessage.contextInfo = { mentionedJid: (message as any).mentions }
        }
